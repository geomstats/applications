...
def main():
    ...
    for option in ["LED","ED", "RD"]:
        ...
        for i in range(1,len(all_graphs)):
            ...
            for j in range(i):
                ...
                if option=="LED": distance[option][i,j] = np.linalg.norm(spd_space.group_log(hat_l1).squeeze(0) - spd_space.group_log(hat_l2).squeeze(0),'fro')
                elif option=="ED": distance[option][i,j] = np.linalg.norm(hat_l1-hat_l2,'fro')
                else: distance[option][i,j] = np.linalg.norm(spd_space.SPDMetric(28).log(hat_l1,hat_l2).squeeze(0) - spd_space.SPDMetric(28).log(hat_l2,hat_l1).squeeze(0),'fro')
    ...
    def fit_kernel_cv( log_euclidean_distance,labels,sigma=None,verbose=False):
        ...
        if sigma is not None: distance=np.exp(-sq(log_euclidean_distance)/(sigma**2))
        ...
        for train_index, test_index in kf.split(range(len(labels))):
            ...
            if verbose: print("Training accuracy:",accuracy_score(labels[train_index],y_train))
    ...
    sigmas=list(np.arange(0.5,10,0.5))+[15,20,25,30,35,40,45,50,60,80,100,120]
    mean_acc={}
    for option in ["LED","ED", "RD"]:
        print("Option: ", option)
        mean_acc[option]=[]
        perf,conf=fit_kernel_cv(distance[option][:T,:T],labels[:T],sigma=None,verbose=False)
        mean_acc[option].append(np.mean([perf[k]['acc'] for k in perf.keys()]))
        for sigma in  sigmas:
                print(sigma)
                perf,conf=fit_kernel_cv(distance[option][:T,:T],labels[:T],sigma,verbose=False)
                mean_acc[option].append(np.mean([perf[k]['acc'] for k in perf.keys()]))
...
if __name__ == "__main__":
	...
	plt.plot(sigmas,mean_acc['LED'][1:], label="Log-Euclidean \nDistance",c="blue")
	plt.plot(sigmas,mean_acc['ED'][1:], label="Euclidean \nDistance",c="black")
	plt.plot(sigmas,mean_acc['RD'][1:], label="Riemannian \nDistance",c="red")
	...
	#plt.savefig("training_CV.pdf", bbox_inches='tight')
	...
	for option in ['LED','ED','RD']:
    	...
    	sb.clustermap(distance2, row_linkage=linkage, col_linkage=linkage,cmap="coolwarm",xticklabels=labels,yticklabels=labels)
    	#plt.savefig("distance_"+str(option)+'.pdf')
