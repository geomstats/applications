...
for isotropic distributions on hyper-spheres of radius 0 < theta < Pi/2 in
...
def multi_plot_modulation_factor(dim, n_expectation=1000, n_theta=20):
    ...
    plt.draw()
    plt.pause(0.01)
    plt.savefig("Figures/SphVarModulation_n10p_d{0}_m{1}.png".format(
        dim, n_expectation))
    plt.savefig("Figures/SphVarModulation_n10p_d{0}_m{1}.pdf".format(
        dim, n_expectation))
    return plt
